...
bool Translator::ResolveImports() {
    ...
    for (; importDescriptor->FirstThunk; ++importDescriptor) {
        ...
        auto remoteOpt = utils::process::get_module(this->Process(), std::wstring(moduleName, moduleName + strlen(moduleName)));
        if (!remoteOpt) {
...
bool Translator::Align(std::vector<Region> &regions,
...
bool Translator::AddSwitchTranslation(Region &rva, PBYTE jumpBuffer,
    ...
    if (indirectJumpTable.RVA) {
        ...
        auto lookupStr = utils::disasm::format(
...
VOID Translator::AddRelativeTranslation(Region &rva, PBYTE instructionBuffer,
    ...
    auto absoluteAddr =
        utils::disasm::absolute(rva.Start(), instruction);
